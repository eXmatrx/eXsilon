...
void GaugeView::drawRect(KDContext * ctx, KDRect rect) const {
  ...
  ctx->fillRect(KDRect(k_indicatorDiameter/2, (height-k_thickness)/2, width*m_level, k_thickness), Palette::Blue);
  ...
  ctx->blendRectWithMask(frame, Palette::Blue, (const uint8_t *)gaugeIndicatorMask, gaugeIndicatorWorkingBuffer);
}
