...
#include "app.h"
#include "exsilon_first_icon.h"
#include "exsilon_second_icon.h"
  ...
  m_selectableTableView(controller, controller, selectionDataSource, controller)
{
  ...
  m_selectableTableView.setMargins(k_topMargin, k_sideMargin, k_bottomMargin, k_sideMargin);
  m_selectableTableView.setBackgroundColor(KDColorWhite);

  m_ImageLogoFirst.setImage(ImageStore::ExsilonFirstIcon);
  m_ImageLogoSecond.setImage(ImageStore::ExsilonSecondIcon);
...
View * Controller::ContentView::subviewAtIndex(int index) {
  ...
  if (index == 0)
    ...
    return &m_ImageLogoFirst;

  return &m_ImageLogoSecond;
}

void Controller::ContentView::layoutSubviews(bool force) {
  m_selectableTableView.setFrame(bounds(), force);
  m_ImageLogoFirst.setFrame(KDRect(82, 45, 81, 35), force);
  m_ImageLogoSecond.setFrame(KDRect(163, 45, 81, 35), force);
}
...
bool Controller::handleEvent(Ion::Events::Event event) {
  if (event == Ion::Events::OK || event == Ion::Events::EXE) {
    ...
    ::App::Snapshot * selectedSnapshot = container->appSnapshotAtIndex(PermutedAppSnapshotIndex(selectionDataSource()->selectedRow() * numberOfColumns() + selectionDataSource()->selectedColumn() + 1));
...
int Controller::numberOfColumns() const {
  AppsContainer * container = AppsContainer::sharedAppsContainer();
  return container->numberOfApps() - 1; // We don't take the 'Home' app into account
}
...
void Controller::tableViewDidChangeSelection(SelectableTableView * t, int previousSelectedCellX, int previousSelectedCellY, bool withinTemporarySelection) {
  ...
  int appIndex = (t->selectedColumn() + t->selectedRow() * numberOfColumns()) + 1;
...
void Controller::tableViewDidChangeSelectionAndDidScroll(SelectableTableView * t, int previousSelectedCellX, int previousSelectedCellY, bool withinTemporarySelection) {
  ...
  if (t->selectedRow() == numberOfRows() - 1) {
    m_view.reloadBottomRow(this, AppsContainer::sharedAppsContainer()->numberOfApps() - 1, numberOfColumns());
  }
}
